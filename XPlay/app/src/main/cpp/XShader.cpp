...
bool XShader::Init(XShaderType type)
{
    ...
    //顶点shader初始化
    mux_.lock();
    vsh_ = InitShader(vertexShader,GL_VERTEX_SHADER);
    if(vsh_ == 0)
    {
        mux_.unlock();
    ...
    switch (type)
    {
        case XSHADER_YUV420P:
            fsh_ = InitShader(fragYUV420P, GL_FRAGMENT_SHADER);
            break;
        case XSHADER_NV12:
            fsh_ = InitShader(fragNV12, GL_FRAGMENT_SHADER);
            break;
        case XSHADER_NV21:
            fsh_ = InitShader(fragNV21, GL_FRAGMENT_SHADER);
            break;
        default:
            mux_.unlock();
    ...
    GLuint apos = (GLuint)glGetAttribLocation(program_, "aPosition");
    ...
    GLuint atex = (GLuint)glGetAttribLocation(program_, "aTexCoord");
    glEnableVertexAttribArray(atex);
    glVertexAttribPointer(atex, 2, GL_FLOAT, GL_FALSE, 8, txts);

    //材质纹理初始化
    //设置纹理层
    glUniform1i( glGetUniformLocation(program_, "yTexture"), 0); //对于纹理第1层
    switch (type)
    {
        case XSHADER_YUV420P:
            glUniform1i(glGetUniformLocation(program_, "uTexture"), 1); //对于纹理第2层
            glUniform1i(glGetUniformLocation(program_, "vTexture"), 2); //对于纹理第3层
            break;
        case XSHADER_NV21:
        case XSHADER_NV12:
            glUniform1i(glGetUniformLocation(program_, "uvTexture"), 1); //对于纹理第2层
            break;
    }
    mux_.unlock();
    XLOGI("初始化Shader成功！");
...
void XShader::GetTexture(unsigned int index, int width,int height, unsigned char *buf, bool is_alpha)
{
    ...
    {
        //材质初始化
        glGenTextures(1, &texts_[index]);

        //设置纹理属性
        glBindTexture(GL_TEXTURE_2D, texts_[index]);
        //缩小的过滤器
...
void XShader::Close()
{
    ...
    //释放材质
    for(int i = 0; i < sizeof(texts_)/sizeof(unsigned int); i++)
    {
        if(texts_[i])
        {
            glDeleteTextures(1, &texts_[i]);
        }
        texts_[i] = 0;
    }
    mux_.unlock();
